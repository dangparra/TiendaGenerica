...
public class clientesDAO {
	...
	public clientesDTO consultar_cliente(clientesDTO cc) {
		...
		try {
			ps=cnn.prepareStatement("SELECT* FROM clientes WHERE cc_cliente=?");
	...
	public int actualizar(clientesDTO cliente) {
		...
		try {
			ps =cnn.prepareStatement("UPDATE clientes SET (dir_cliente=?, email_cliente=?, nombre_cliente=?, telefono_cliente=?) WHERE cc_cliente=?");	
			
			
			ps.setString(2, cliente.getDir_cliente());
			ps.setString(3, cliente.getEmail_cliente());
			ps.setString(4, cliente.getNombre_cliente());
			ps.setString(5, cliente.getNombre_cliente());
			ps.setLong(1, cliente.getCc_cliente());
			x=ps.executeUpdate();
